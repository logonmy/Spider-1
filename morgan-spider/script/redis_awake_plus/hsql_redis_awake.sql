...
--stg.redis_awake_temp
drop table if exists stg.redis_awake_temp;

create table if not exists stg.redis_awake_temp(
	key1 string,
	key2 string,
	mobile string
);

insert into stg.redis_awake_temp
            ...
            (
                ...
                (
                    ...
                    (
                        ...
                        concat
                        (
                          first_letter,birth_year,gender,
                        ...
                        where
                        first_letter is not null and first_letter rlike '[\\u4e00-\\u9fa5]'
...
--default.redis_awake_final_temp
drop table if exists default.redis_awake_final_temp;

create table if not exists default.redis_awake_final_temp(
	id bigint,
	key1 string,
	key2 string,
	mobiles string
...
insert overwrite table default.redis_awake_final_temp partition(rand_)
select
...
(
  ...
  from stg.redis_awake_temp
  group by key1,key2
)as t1
cross join
(
    select coalesce(max(id),0) max_id from default.redis_awake_final_temp
) t3;
