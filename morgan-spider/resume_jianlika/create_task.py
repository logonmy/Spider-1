...
def create_task_from_mysql(use_keyword='0'):
    ...
    cur = conn.cursor()
    city_number = cur.execute('select * from city_entrence where source="RESUME_KA" and valid=1')
    cities = cur.fetchall()
    function_number = cur.execute('select * from function_entrence where source="RESUME_KA" and valid=1')
    functions = cur.fetchall()
    logger.info('获取到%s 个城市, %s 个职能' % (city_number, function_number))
    ...
    for city in city_result:
        for function in functions:
            add_task_data = {
                "callSystemID": 'morgan-jianlika-resume-1',
                "source": 'RESUME_KA',
                "traceID": str(uuid.uuid1()),
                # "executeParam": json.loads(i.strip()), 
                "executeParam": json.dumps(
                    {'hJobs': function[8], 'keywords': function[7], 'areas': city[6], 'areas_name': city[1],
