...
class ADB():
    ...
    def __build_command__(self, cmd):
        ...
        after_cmd = self.__adb_path + ' ' + cmd if self.__target is None else self.__adb_path + ' ' + ' -s ' + self.__target + ' ' + cmd
        print u'execute cmd >>> ', after_cmd
        return after_cmd
    ...
    def run_cmd(self, cmd):
        ...
        try:
            ...
            pass
        print u'cmd return <<< ', self.__output
        return
    ...
    def shell_touch(self, cmd, rate):
        """
        Executes a shell command
        adb shell <cmd>
        """
        self.__clean__()
        match_ = re.search('^input tap(\\d+).*?(\\d+)$', cmd)
        if match_:
            first_ = match_.group(1)
            second_ = match_.group(2)
            cmd = 'input tap %s %s' % (int(first_) * rate, int(second_) * rate)
        self.run_cmd('shell %s' % cmd)
        return self.__output
